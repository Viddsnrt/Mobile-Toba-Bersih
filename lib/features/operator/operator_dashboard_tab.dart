...
class _OperatorDashboardTabState extends State<OperatorDashboardTab> {
  ...
  Future<void> _fetchMyTasks() async {
    ...
    try {
      // 🔥 PERBAIKAN: Ubah endpoint menjadi /api/penugasan?driverId=...
      final response = await http.get(
        Uri.parse('http://$ipAddress:5000/api/penugasan?driverId=${widget.driverId}'),
      );
  ...
  Widget _buildTaskCard(Map<String, dynamic> task) {
    ...
    if (statusStr == 'BEKERJA' || statusStr == 'DALAM_PERJALANAN')
      statusColor = Colors.amber.shade700;

    double lat = task['latitude'] != null
        ? double.tryParse(task['latitude'].toString()) ?? 2.3333
        : 2.3333;
    double lng = task['longitude'] != null
        ? double.tryParse(task['longitude'].toString()) ?? 99.0667
        : 99.0667;
                  ...
                  final result = await Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) => DriverMapScreen(
                        taskId: task['id']
                            .toString(), // 🔥 WAJIB KIRIM ID TUGAS KE PETA
                        destinationLat: lat,
                        destinationLng: lng,
